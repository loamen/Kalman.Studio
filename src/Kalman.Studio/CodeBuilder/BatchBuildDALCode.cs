...
{
    public partial class BatchBuildDALCode : Form
    {
        SODatabase currentDatabase;
        List<SOTable> tableList = new List<SOTable>();
        string templatePath = Path.Combine(Application.StartupPath, "T4Template\\DAL");

        public BatchBuildDALCode(SODatabase db)
        {
            InitializeComponent();
            currentDatabase = db;
        ...
        private void BatchBuildDALCode_Load(object sender, EventArgs e)
        {
            ...
            txtOutputPath.Text = Path.Combine(Application.StartupPath, "Output\\DALCode");
        }
